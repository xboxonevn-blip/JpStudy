...
void main() {
  ...
  group('ImmersionArticle / reading bank', () {
    ...
    test('ImmersionService loads a canonical local reading bank', () async {
      ...
      expect(
        articles.any(
          (article) => article.id == 'n5-lesson-01' && article.officialLevel == 'N5',
        ),
        isTrue,
      );
      expect(
        articles.any(
          (article) => article.id == 'n4-lesson-26' && article.officialLevel == 'N4',
        ),
        isTrue,
      );
      expect(
        articles.any(
          (article) => article.id == 'n3-lesson-51' && article.officialLevel == 'N3',
        ),
        isTrue,
      );
    });
  ...
  group('read status', () {
    ...
    test('read ids are deduplicated because service stores them as a Set',
        () async {
      final service = ImmersionService();
  ...
  group('quiz history', () {
    ...
    test('saveQuizAttempt is a no-op when total <= 0', () async {
      ...
      await service.saveQuizAttempt(articleId: 'article_1', correct: 2, total: 0);
    ...
    test('newest attempts are inserted first', () async {
      ...
      await service.saveQuizAttempt(articleId: 'article_1', correct: 1, total: 3);
      await service.saveQuizAttempt(articleId: 'article_1', correct: 2, total: 3);
      final history = await service.getQuizHistory('article_1');
    ...
    test('getQuizHistory respects read limit argument', () async {
      ...
      for (var i = 0; i < 4; i++) {
        await service.saveQuizAttempt(articleId: 'article_1', correct: i, total: 4);
      }
    ...
    test('getQuizHistory filters out corrupted entries with total <= 0', () async {
      SharedPreferences.setMockInitialValues({
        'immersion_quiz_history_v1': jsonEncode({
          'article_1': [
            {'correct': 2, 'total': 0, 'attemptedAt': '2026-02-01T10:00:00.000'},
            {'correct': 1, 'total': 3, 'attemptedAt': '2026-02-01T11:00:00.000'},
          ],
        }),
      });
      final service = ImmersionService();
    ...
    test('getQuizHistory returns empty when decoded payload is not a map', () async {
      SharedPreferences.setMockInitialValues({
    ...
    test('saveQuizAttempt recovers from invalid existing JSON by rebuilding payload',
        () async {
      SharedPreferences.setMockInitialValues({
        'immersion_quiz_history_v1': 'not-json',
      });
    ...
    test('histories for different articles are isolated', () async {
      ...
      await service.saveQuizAttempt(articleId: 'article_1', correct: 2, total: 3);
      await service.saveQuizAttempt(articleId: 'article_2', correct: 1, total: 2);
