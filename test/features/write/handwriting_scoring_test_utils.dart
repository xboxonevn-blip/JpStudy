...
StrokeSequence buildStrokesFromTemplate(
  ...
  final indices =
      order ?? List<int>.generate(template.strokes.length, (i) => i);
...
Map<String, StrokeSequence> buildRegressionNegativeCases(
  ...
  StrokeSequence reorder(List<int> order) => [
    for (final i in order) [...base[i]],
  ];
  ...
  StrokeSequence dropLast(StrokeSequence source) =>
      source.sublist(0, max(1, source.length - 1));
  ...
  return <String, StrokeSequence>{
    'order_reverse_all': reorder(
      List<int>.generate(base.length, (i) => base.length - 1 - i),
    ),
    'order_swap_first_two': reorder(_swapOrder(base.length, 0, 1)),
    'order_swap_mid': reorder(
      hasMid ? _swapOrder(base.length, 1, 2) : _rotateLeft(base.length),
    ),
    ...
    'direction_reverse_first': hasPair
        ? [
            base[0].reversed.toList(),
            ...base.sublist(1).map((stroke) => [...stroke]),
          ]
        : reverseEach(base),
    'direction_reverse_last': hasPair
        ? [
            ...base.sublist(0, base.length - 1).map((stroke) => [...stroke]),
            base.last.reversed.toList(),
          ]
        : reverseEach(base),
    'mirror_horizontal': mapPoints(
      base,
      (p) => Offset((center.dx * 2) - p.dx, p.dy),
    ),
    'mirror_vertical': mapPoints(
      base,
      (p) => Offset(p.dx, (center.dy * 2) - p.dy),
    ),
    'shape_scale_wide': mapPoints(
      base,
      (p) => _scaleAround(p, center, 1.7, 0.58),
    ),
    'shape_scale_tall': mapPoints(
      base,
      (p) => _scaleAround(p, center, 0.58, 1.7),
    ),
    'shape_too_small': mapPoints(
      base,
      (p) => _scaleAround(p, center, 0.2, 0.2),
    ),
    'shape_too_large': mapPoints(
      base,
      (p) => _scaleAround(p, center, 1.85, 1.85),
    ),
    ...
    'endpoint_shift_start': [
      [base[0].first + const Offset(56, -56), ...base[0].skip(1)],
      ...base.sublist(1).map((stroke) => [...stroke]),
    ],
    'endpoint_shift_end': [
      [
...
StrokePath _linePath(Offset start, Offset end, {int points = 9}) {
...
List<int> _rotateLeft(int length) {
  ...
  return [...List<int>.generate(length - 1, (i) => i + 1), 0];
}

List<int> _rotateRight(int length) {
  if (length <= 1) return List<int>.generate(length, (i) => i);
  return [length - 1, ...List<int>.generate(length - 1, (i) => i)];
}
