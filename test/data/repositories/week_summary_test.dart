...
void main() {
  ...
  group('weekSummaryProvider', () {
    ...
    test(
      'computes accuracy from only attempts started within the last 7 days',
      () async {
        final now = DateTime.now();
        // inside cutoff
        await insertAttempt(
    ...
    test(
      'attempt exactly 7 days old is excluded because code uses isAfter(cutoff)',
      () async {
        final now = DateTime.now();
