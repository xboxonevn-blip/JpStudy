...
void main() {
  ...
  group('RecoveryPack.fromJson', () {
    ...
    test('parses valid json', () {
      final pack = RecoveryPack.fromJson(_valid());
  ...
  group('RecoveryPack.isFresh', () {
    RecoveryPack _pack(DateTime createdAt) => RecoveryPack(
          source: 'mock_exam',
          lessonTitle: 'L1',
          termIds: [1],
          createdAt: createdAt,
    ...
    test('fresh when created now', () {
      expect(_pack(DateTime.now()).isFresh, isTrue);
    });

    test('fresh when created 1 day ago', () {
      expect(_pack(DateTime.now().subtract(const Duration(days: 1))).isFresh, isTrue);
    });

    test('stale when created 3 days ago', () {
      expect(_pack(DateTime.now().subtract(const Duration(days: 3))).isFresh, isFalse);
    });
  });
