        ...
        child: Column(
          ...
          children: [
            AppFeatureCard(
              ...
              onPrimaryTap: () => context.push('/me'),
              secondaryLabel: _dataLabel(language),
              onSecondaryTap: () => context.push('/me/data'),
            ),
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              ...
              AppMetricPill(label: _focusLabel(language), value: _focusValue(language)),
              AppMetricPill(label: _modeLabel(language), value: _modeValue(language)),
              AppMetricPill(label: _feedbackLabel(language), value: _feedbackValue(language)),
            ],
          ),
        ],
      ),
...
List<_ShortcutItem> _shortcuts(AppLanguage language) => [
      _ShortcutItem(Icons.person_rounded, _profileTitle(language), _profileSubtitle(language), (context) => context.push('/me')),
      _ShortcutItem(Icons.storage_rounded, _storageTitle(language), _storageSubtitle(language), (context) => context.push('/me/data')),
      _ShortcutItem(Icons.palette_rounded, _designLabTitle(language), _designLabSubtitle(language), (context) => context.push('/design-lab'), status: _labStatus(language), tone: AppStatusTone.neutral),
    ];

List<_RoadmapItem> _roadmapItems(AppLanguage language) => switch (language) {
      AppLanguage.en => const [
          _RoadmapItem(Icons.check_circle_rounded, 'Shell and navigation refresh', 'Completed foundation for the 10-section information architecture.', 'Done', AppStatusTone.success),
          _RoadmapItem(Icons.draw_rounded, 'Kanji workspace and radicals', 'Expanded study-first kanji experience with draw search and 214 radicals.', 'Live', AppStatusTone.primary),
          _RoadmapItem(Icons.groups_rounded, 'Community loops', 'Invite cards, challenge rooms, and social proof surfaces are next.', 'Next', AppStatusTone.warning),
        ],
      AppLanguage.vi => const [
          _RoadmapItem(Icons.check_circle_rounded, 'Làm mới shell và điều hướng', 'Đã hoàn thành nền tảng cho kiến trúc thông tin 10 mục.', 'Xong', AppStatusTone.success),
          _RoadmapItem(Icons.draw_rounded, 'Workspace Kanji và bộ thủ', 'Mở rộng trải nghiệm học kanji với draw search và 214 bộ thủ.', 'Đang chạy', AppStatusTone.primary),
          _RoadmapItem(Icons.groups_rounded, 'Vòng lặp cộng đồng', 'Invite card, challenge room và social proof là bước tiếp theo.', 'Tiếp theo', AppStatusTone.warning),
        ],
      AppLanguage.ja => const [
          _RoadmapItem(Icons.check_circle_rounded, 'shell と navigation の刷新', '10セクション情報設計の土台を完了しました。', '完了', AppStatusTone.success),
          _RoadmapItem(Icons.draw_rounded, 'Kanji workspace と部首', 'draw search と 214 部首で kanji 体験を広げました。', '稼働中', AppStatusTone.primary),
          _RoadmapItem(Icons.groups_rounded, 'community loop', 'invite card、challenge room、social proof が次です。', '次', AppStatusTone.warning),
        ],
    };

List<_ConnectItem> _connectItems(AppLanguage language) => switch (language) {
      AppLanguage.en => const [
          _ConnectItem(Icons.feedback_rounded, 'Send feedback', 'Bundle UX notes, bug reports, and feature requests in one surface.', 'Open', AppStatusTone.success, 'Feedback intake will be wired to a real channel later.'),
          _ConnectItem(Icons.campaign_rounded, 'Invite a friend', 'Prepare a lightweight referral path without changing the app shell.', 'Soon', AppStatusTone.warning, 'Referral flow is still local placeholder content.'),
          _ConnectItem(Icons.groups_rounded, 'Community room', 'Reserve a future home for live learners, sprint rooms, and announcements.', 'Planned', AppStatusTone.neutral, 'Community links will be enabled in a later release.'),
        ],
      AppLanguage.vi => const [
          _ConnectItem(Icons.feedback_rounded, 'Gửi phản hồi', 'Gom góp ý UX, bug report và feature request vào một chỗ.', 'Mở', AppStatusTone.success, 'Kênh nhận phản hồi thật sẽ được nối sau.'),
          _ConnectItem(Icons.campaign_rounded, 'Mời bạn bè', 'Chuẩn bị lối giới thiệu nhẹ mà không đổi app shell.', 'Sớm', AppStatusTone.warning, 'Flow giới thiệu hiện vẫn là placeholder local.'),
          _ConnectItem(Icons.groups_rounded, 'Phòng cộng đồng', 'Chừa sẵn nơi cho người học trực tiếp, sprint room và thông báo.', 'Kế hoạch', AppStatusTone.neutral, 'Link cộng đồng sẽ được bật ở bản sau.'),
        ],
      AppLanguage.ja => const [
          _ConnectItem(Icons.feedback_rounded, 'フィードバック送信', 'UX メモ、bug report、feature request を1か所にまとめます。', '受付中', AppStatusTone.success, 'feedback 導線は後で実チャネルへ接続します。'),
          _ConnectItem(Icons.campaign_rounded, '友達を招待', 'app shell を変えずに紹介導線を準備します。', '近日', AppStatusTone.warning, '紹介 flow はまだローカル placeholder です。'),
          _ConnectItem(Icons.groups_rounded, 'コミュニティルーム', '学習者の live room、sprint room、announcement の居場所を確保します。', '予定', AppStatusTone.neutral, 'community link は後続リリースで有効になります。'),
        ],
    };
...
String _heroTitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'One calm place for profile, settings, and community touchpoints',
...
String _heroSubtitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'This screen now works like a real control center instead of only being a list of links.',
      AppLanguage.vi => 'Màn này giờ hoạt động như một control center thật thay vì chỉ là danh sách link.',
...
String _profileSubtitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Review progress snapshots, challenges, and personal bests.',
...
String _designLabSubtitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Inspect visual experiments and keep the app style coherent.',
...
String _roadmapCaption(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Short product roadmap so this page feels like a living hub.',
      AppLanguage.vi => 'Roadmap ngắn để trang này có cảm giác là một hub đang sống.',
...
String _connectCaption(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Prepared surfaces for feedback, referrals, and future social loops.',
      AppLanguage.vi => 'Các bề mặt được chuẩn bị cho feedback, referral và social loop sau này.',
...
class _ShortcutItem {
  const _ShortcutItem(this.icon, this.title, this.subtitle, this.onTap,
      {this.status, this.tone = AppStatusTone.neutral});
...
class _RoadmapItem {
  const _RoadmapItem(this.icon, this.title, this.subtitle, this.status, this.tone);
...
class _ConnectItem {
  const _ConnectItem(this.icon, this.title, this.subtitle, this.status, this.tone, this.feedback);
