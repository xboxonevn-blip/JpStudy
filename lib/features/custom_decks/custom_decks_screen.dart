        ...
        child: Column(
          ...
          children: [
            AppFeatureCard(
              ...
              onSecondaryTap: () => _showSnack(context, _quickQuizSoon(language)),
            ),
                ...
                children: [
                    ...
                    children: [
                      for (var i = 0; i < recipes.length; i++)
                        ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => _selectedRecipe = i),
                        ),
                    ],
                  ),
                  ...
                  LayoutBuilder(
                    builder: (context, constraints) {
                      ...
                      final overview = _RecipeOverviewCard(recipe: recipe, language: language);
                      final queue = _RecipeQueueCard(recipe: recipe, language: language);
                      if (stacked) {
                        return Column(
                          children: [
                            overview,
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: AppSpacing.sm,
            children: [
              AppMetricPill(label: _durationLabel(language), value: recipe.duration),
              AppMetricPill(label: _focusLabel(language), value: recipe.focus),
              AppMetricPill(label: _energyLabel(language), value: recipe.energy),
            ],
          ),
          const SizedBox(height: AppSpacing.lg),
    ...
    return AppSectionCard(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSpacing.md),
          AppProgressStrip(value: recipe.progress, label: recipe.progressLabel(language)),
          const SizedBox(height: AppSpacing.md),
...
List<_StudyRecipe> _recipes(
  ...
  final dueTotal = (dashboard?.vocabDue ?? 0) +
  ...
  return switch (language) {
      AppLanguage.en => [
          _StudyRecipe('Night cram', 'Fast catch-up session for overdue cards and weak items.', nextUp != null ? '${12 + nextUp.dueCount} min' : '18 min', 'Recall', 'High', 'Power', dueTotal > 0 ? 0.62 : 0.28, [
            nextUp != null ? 'Start from ${nextUp.title} because it has the highest immediate pressure.' : 'Pull the most overdue items first.',
            continueLabel != null ? 'Use "$continueLabel" as the first lane so this workspace mirrors your home priority.' : 'Mix mistakes and fast flashcards into one loop.',
            'End with a 3-minute confidence recap.',
          ], [
            _QueueItem(Icons.psychology_alt_rounded, 'Due queue', '${continueCount == 0 ? dueTotal : continueCount} items need quick attention.', 'Due', AppStatusTone.warning),
            _QueueItem(Icons.layers_rounded, 'Active decks', '$activeCount active deck${activeCount == 1 ? '' : 's'} available right now.', 'Decks', AppStatusTone.success),
          ]),
          _StudyRecipe('Mixed sprint', 'Blend kanji, vocab, and grammar into one focused block.', activeCount > 0 ? '${18 + activeCount * 2} min' : '22 min', 'Mixed', 'Balanced', 'New', activeCount > 0 ? 0.78 : 0.5, [
            nextUp != null ? 'Warm up with ${nextUp.title} before switching lanes.' : 'Warm up with kanji recognition.',
            'Switch into vocab recall and one grammar drill.',
            completedCount > 0 ? 'Reuse patterns from $completedCount completed deck${completedCount == 1 ? '' : 's'} for a faster sprint.' : 'Finish with a short active review burst.',
          ], [
            _QueueItem(Icons.grid_view_rounded, 'Kanji lane', '${dashboard?.kanjiDue ?? 12} recognition prompts.', 'Lane 1', AppStatusTone.primary),
            _QueueItem(Icons.translate_rounded, 'Vocab lane', '${dashboard?.vocabDue ?? 18} high-frequency prompts.', 'Lane 2', AppStatusTone.success),
          ]),
          _StudyRecipe('Speaking loop', 'Build a compact loop for shadowing and speaking confidence.', '15 min', 'Output', 'Medium', 'Beta', dashboard == null ? 0.34 : (dashboard.totalMistakeCount > 0 ? 0.51 : 0.24), [
            'Start with phrase cards.',
            'Repeat two short shadowing cycles.',
            dashboard != null && dashboard.totalMistakeCount > 0 ? 'Mark ${dashboard.totalMistakeCount} weak items for tomorrow so they flow back into your recovery loop.' : 'Mark low-confidence phrases for tomorrow.',
          ], [
            _QueueItem(Icons.mic_rounded, 'Phrase loop', '${(dashboard?.grammarDue ?? 4) + 4} useful lines for speaking practice.', 'Voice', AppStatusTone.neutral),
            _QueueItem(Icons.hearing_rounded, 'Shadowing', '${(dashboard?.kanjiMistakeCount ?? 0) > 0 ? 3 : 2} micro audio drills.', 'Audio', AppStatusTone.primary),
          ]),
        ],
      AppLanguage.vi => [
          _StudyRecipe('Nhồi tối', 'Phiên bắt kịp nhanh cho thẻ quá hạn và mục yếu.', nextUp != null ? '${12 + nextUp.dueCount} phút' : '18 phút', 'Recall', 'Cao', 'Mạnh', dueTotal > 0 ? 0.62 : 0.28, [
            nextUp != null ? 'Bắt đầu từ ${nextUp.title} vì deck này đang chịu áp lực cao nhất.' : 'Kéo các mục quá hạn nhất lên trước.',
            continueLabel != null ? 'Dùng "$continueLabel" làm làn đầu để workspace này bám đúng ưu tiên ở màn chính.' : 'Trộn mistake và flashcard nhanh vào một vòng.',
            'Kết thúc bằng recap tự tin 3 phút.',
          ], [
            _QueueItem(Icons.psychology_alt_rounded, 'Hàng due', '${continueCount == 0 ? dueTotal : continueCount} mục cần xử lý nhanh.', 'Đến hạn', AppStatusTone.warning),
            _QueueItem(Icons.layers_rounded, 'Deck đang chạy', '$activeCount deck đang hoạt động lúc này.', 'Deck', AppStatusTone.success),
          ]),
          _StudyRecipe('Sprint hỗn hợp', 'Trộn kanji, từ vựng và ngữ pháp trong một block tập trung.', activeCount > 0 ? '${18 + activeCount * 2} phút' : '22 phút', 'Hỗn hợp', 'Cân bằng', 'Mới', activeCount > 0 ? 0.78 : 0.5, [
            nextUp != null ? 'Khởi động bằng ${nextUp.title} trước khi chuyển làn.' : 'Khởi động bằng nhận diện kanji.',
            'Chuyển sang recall từ vựng và một drill ngữ pháp.',
            completedCount > 0 ? 'Tái dùng pattern từ $completedCount deck đã xong để sprint mượt hơn.' : 'Kết bằng một burst ôn chủ động ngắn.',
          ], [
            _QueueItem(Icons.grid_view_rounded, 'Làn kanji', '${dashboard?.kanjiDue ?? 12} prompt nhận diện.', 'Làn 1', AppStatusTone.primary),
            _QueueItem(Icons.translate_rounded, 'Làn từ vựng', '${dashboard?.vocabDue ?? 18} prompt tần suất cao.', 'Làn 2', AppStatusTone.success),
          ]),
          _StudyRecipe('Vòng nói', 'Tạo vòng ngắn cho shadowing và tăng tự tin khi nói.', '15 phút', 'Output', 'Vừa', 'Beta', dashboard == null ? 0.34 : (dashboard.totalMistakeCount > 0 ? 0.51 : 0.24), [
            'Bắt đầu bằng phrase card.',
            'Lặp hai chu kỳ shadowing ngắn.',
            dashboard != null && dashboard.totalMistakeCount > 0 ? 'Đánh dấu ${dashboard.totalMistakeCount} mục yếu cho ngày mai để quay lại recovery loop.' : 'Đánh dấu các câu chưa tự tin cho ngày mai.',
          ], [
            _QueueItem(Icons.mic_rounded, 'Vòng câu', '${(dashboard?.grammarDue ?? 4) + 4} câu hữu ích để luyện nói.', 'Giọng nói', AppStatusTone.neutral),
            _QueueItem(Icons.hearing_rounded, 'Luyện nhại', '${(dashboard?.kanjiMistakeCount ?? 0) > 0 ? 3 : 2} drill âm thanh nhỏ.', 'Âm thanh', AppStatusTone.primary),
          ]),
        ],
      AppLanguage.ja => [
          _StudyRecipe('Night cram', '期限切れカードと弱点項目を素早く回す補強セッションです。', nextUp != null ? '${12 + nextUp.dueCount}分' : '18分', 'Recall', '高', '強化', dueTotal > 0 ? 0.62 : 0.28, [
            nextUp != null ? '最初は ${nextUp.title} から始め、もっとも圧力の高い deck を先に処理します。' : '最も overdue な項目から先に回します。',
            continueLabel != null ? 'ホームの優先と合わせるため、最初の lane は "$continueLabel" を使います。' : 'mistake と高速 flashcard を1つの loop にまとめます。',
            '最後に 3 分の confidence recap を行います。',
          ], [
            _QueueItem(Icons.psychology_alt_rounded, 'Due queue', '${continueCount == 0 ? dueTotal : continueCount} 件を先に処理します。', '期限', AppStatusTone.warning),
            _QueueItem(Icons.layers_rounded, 'Active decks', '$activeCount 件の active deck を利用できます。', 'Deck', AppStatusTone.success),
          ]),
          _StudyRecipe('Mixed sprint', 'kanji、vocab、grammar を1つの集中 block にまとめます。', activeCount > 0 ? '${18 + activeCount * 2}分' : '22分', 'Mixed', 'バランス', '新着', activeCount > 0 ? 0.78 : 0.5, [
            nextUp != null ? '${nextUp.title} でウォームアップしてから lane を切り替えます。' : 'kanji recognition でウォームアップ。',
            'vocab recall と grammar drill へ切り替えます。',
            completedCount > 0 ? '$completedCount 件の completed deck の型を使って sprint を速くします。' : '短い active review burst で締めます。',
          ], [
            _QueueItem(Icons.grid_view_rounded, 'Kanji lane', '${dashboard?.kanjiDue ?? 12} 個の recognition prompt。', 'Lane 1', AppStatusTone.primary),
            _QueueItem(Icons.translate_rounded, 'Vocab lane', '${dashboard?.vocabDue ?? 18} 個の高頻度 prompt。', 'Lane 2', AppStatusTone.success),
          ]),
          _StudyRecipe('Speaking loop', 'shadowing と発話の自信を高めるコンパクトな loop です。', '15分', 'Output', '中', 'Beta', dashboard == null ? 0.34 : (dashboard.totalMistakeCount > 0 ? 0.51 : 0.24), [
            'phrase card から始めます。',
            '短い shadowing cycle を2回行います。',
            dashboard != null && dashboard.totalMistakeCount > 0 ? '${dashboard.totalMistakeCount} 件の弱点を明日に回し、recovery loop へ戻します。' : '自信の低い phrase を明日に回します。',
          ], [
            _QueueItem(Icons.mic_rounded, 'フレーズループ', '${(dashboard?.grammarDue ?? 4) + 4} フレーズを話す練習に使います。', '音声', AppStatusTone.neutral),
            _QueueItem(Icons.hearing_rounded, 'シャドーイング', '${(dashboard?.kanjiMistakeCount ?? 0) > 0 ? 3 : 2} 個の micro audio drill。', '音源', AppStatusTone.primary),
          ]),
        ],
    };
}
...
List<_ToolkitItem> _toolkit(AppLanguage language) => switch (language) {
      AppLanguage.en => [
          _ToolkitItem(Icons.layers_clear_rounded, 'Cram mode', 'Override the usual due queue for one urgent session tonight.', 'Power', AppStatusTone.warning, '', cramMode: true),
          const _ToolkitItem(Icons.quiz_rounded, 'Custom quiz', 'Blend kanji, vocab, and grammar into one targeted set.', 'New', AppStatusTone.success, 'Custom quiz builder is still local-only for now.'),
          const _ToolkitItem(Icons.repeat_rounded, 'Flashcard loop', 'Run a rapid loop for pronunciation, recall, and confidence rating.', 'Loop', AppStatusTone.primary, 'Loop settings will be connected to saved presets later.'),
        ],
      AppLanguage.vi => [
          _ToolkitItem(Icons.layers_clear_rounded, 'Chế độ nhồi nhanh', 'Ghi đè hàng due thông thường cho một phiên gấp tối nay.', 'Mạnh', AppStatusTone.warning, '', cramMode: true),
      ...
      AppLanguage.ja => [
          _ToolkitItem(Icons.layers_clear_rounded, 'Cram mode', '今夜だけ通常の due queue を上書きして急ぎの session を作ります。', '強化', AppStatusTone.warning, '', cramMode: true),
...
String _heroTitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Build focused sessions without leaving the current app flow',
      AppLanguage.vi => 'Tạo phiên học tập trung mà không rời flow hiện tại của app',
      AppLanguage.ja => '今の app flow を崩さずに集中 session を組み立てる',
    };
String _heroSubtitle(AppLanguage language) => switch (language) {
      AppLanguage.en => 'This screen now behaves like an active study workspace with recipes, queue logic, and starter packs.',
      AppLanguage.vi => 'Màn này giờ giống một workspace học chủ động với recipe, queue logic và starter pack.',
      AppLanguage.ja => 'recipe、queue logic、starter pack を備えた active study workspace として機能します。',
    };
...
String _recipeCaption(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Choose a ready-made flow and inspect how the queue would behave.',
...
String _toolkitCaption(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Shortcuts for targeted practice without changing the shell.',
...
String _templatesCaption(AppLanguage language) => switch (language) {
      AppLanguage.en => 'Small packs that make this screen feel stocked instead of empty.',
      AppLanguage.vi => 'Các gói nhỏ giúp màn này có cảm giác đầy đặn thay vì trống.',
...
class _StudyRecipe {
  const _StudyRecipe(this.name, this.subtitle, this.duration, this.focus,
      this.energy, this.badge, this.progress, this.steps, this.queueItems);
...
class _QueueItem {
  const _QueueItem(this.icon, this.title, this.subtitle, this.status, this.tone);
