...
class _DataSettingsScreenState extends ConsumerState<DataSettingsScreen> {
        ...
        data: (status) => Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: Text(
                controller.cloudSyncStatusSubtitle(context, language, status),
              ),
            ),
            Text(
            ...
            Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: settings.isReady && status.isLinked
                      ? () =>
                            controller.downloadFromCloudFile(context, language)
                      : null,
                ...
                if (status.isLinked)
                  OutlinedButton.icon(
                    onPressed: settings.isReady
                        ? () =>
                              controller.unlinkCloudSyncFile(context, language)
                        : null,
  ...
  String _subtitle(AppLanguage language) {
    switch (language) {
      ...
      case AppLanguage.ja:
        return '自動バックアップ、入出力、連携中の同期ファイルをここで管理します。';
    }
  }
