...
class RadicalRelatedLevelGroup {
  const RadicalRelatedLevelGroup({
    required this.level,
    required this.items,
  });
...
class RadicalRelatedKanjiSummary {
  ...
  List<String> get allCharacters => [for (final item in allItems) item.character];
...
RadicalRelatedKanjiSummary buildRelatedKanjiSummary(
  ...
  for (final item in kanjiItems) {
    ...
    if (allItems.any((existing) => existing.character == item.character)) continue;
  ...
  for (final entry in grouped.entries) {
    if (groups.any((group) => group.level == entry.key) || entry.value.isEmpty) {
      continue;
    }
