...
class PracticeScreen extends ConsumerWidget {
        ...
        child: Column(
          ...
          children: [
            AppFeatureCard(
              ...
              onPrimaryTap: () => _openPrimaryGoal(context, dueCount, mistakeCount),
              secondaryLabel: _searchLabel(language),
              onSecondaryTap: () => context.push('/search'),
              status: AppStatusChip(
                label: dueCount > 0 ? _dueBadge(language, dueCount) : _readyBadge(language),
                tone: dueCount > 0 ? AppStatusTone.warning : AppStatusTone.success,
              ),
            ),
            ...
            _GoalTile(
              ...
              status: AppStatusChip(
                label: '$mistakeCount',
                tone: mistakeCount > 0 ? AppStatusTone.warning : AppStatusTone.neutral,
              ),
            ),
            const SizedBox(height: 10),
            _GoalTile(
  ...
  String _subtitle(AppLanguage language) => switch (language) {
    AppLanguage.en => 'A young, calm study hub built around intent instead of noisy feature lists.',
    AppLanguage.vi => 'Một study hub trẻ, gọn và bình tĩnh, ưu tiên mục tiêu học thay vì danh sách tính năng rối.',
    AppLanguage.ja => '機能の羅列ではなく、学習目的から選べる落ち着いたスタディハブです。',
  };

  String _primaryCta(AppLanguage language) => switch (language) {
    AppLanguage.en => 'Start best next step',
    AppLanguage.vi => 'Bắt đầu bước tốt nhất',
    AppLanguage.ja => '最適な次の一歩を開始',
  };
  ...
  String _goalDueSubtitle(AppLanguage language, int dueCount) => switch (language) {
  ...
  String _goalFixSubtitle(AppLanguage language, int count) => switch (language) {
    AppLanguage.en => count > 0 ? '$count weak items still need work.' : 'No weak items are waiting.',
    AppLanguage.vi => count > 0 ? '$count mục yếu vẫn cần xử lý.' : 'Hiện chưa có mục yếu chờ xử lý.',
