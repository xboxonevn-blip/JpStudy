...
final studyHubDecksProvider = FutureProvider.autoDispose<StudyHubDecksBoard>((
  ref,
) async {
...
StudyHubDecksBoard buildStudyHubDecksBoard(List<LessonMeta> meta) {
  ...
  final decks = meta
      .map((m) {
        final progress = m.termCount > 0
            ? ((m.completedCount / m.termCount) * 100).round()
            : 0;
      ...
      .where((d) => d.progressPercent > 0 || d.dueCount > 0)
      .toList();
  ...
  final nextUp =
      decks.isNotEmpty && (!decks.first.isFinished || decks.first.dueCount > 0)
      ? decks.first
      : null;
  final activeDecks = decks
      .where((d) => !d.isFinished && d.id != nextUp?.id)
      .toList();
  final completedDecks = decks
      .where((d) => d.isFinished && d.id != nextUp?.id)
      .toList();
