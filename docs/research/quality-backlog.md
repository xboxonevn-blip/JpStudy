...
| QA-A-002 | App defects | P1 | Live retest: VI UI leaked `Ready now`, `Companion`, `14 chapter`, `Catalog`, `review`; post-deploy sweep still found `Companion` in Minna status badges. | Fixed locally | Added widget regression for Vietnamese vocab catalog badges; focused vocab test passed. Live proof pending after deploy. |
