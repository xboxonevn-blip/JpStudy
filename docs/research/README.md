...
- D8 launch-readiness aggregate: `npm run report:launch-readiness -- --json` added on 2026-05-16. Latest Storage-aware run on 2026-05-17T02:49+07 returns `complete=false` without `storage-not-provisioned`; unresolved gates are legal approval, Sentry DSN/first issue, executed deletion proof, GA4 retention proof, BigQuery-exported learning-outcome GA4 rows, and later App Check enforcement. Current launch/storage/deletion/GA4 reports print operator URLs so handoff is direct, but URLs do not close proof gates. Client-side live emission of all three learning events is now proven by Playwright network capture.
