...
Current Kanji per-language UX status: Hán-Việt rules are Kanji-owned and visible only in Vietnamese. Kanji detail, search keywords, lesson Kanji list, reading practice, and handwriting practice now hide Hán-Việt/Vietnamese-only aids for EN/JA and fall back to English meanings where no localized Japanese definition exists. Deployed `4747b677`; live proof covers JA Kanji detail and JA Search `nhan` gating. Remaining gap: live proof for lesson/practice consumer screens and real Japanese definition data.
