...
- D8 launch-readiness aggregate: `npm run report:launch-readiness -- --json` added on 2026-05-16. Latest run returns `complete=false` and lists the expected unresolved gates: legal approval, Sentry DSN/first issue, Storage provisioning/migration proof, executed deletion proof, GA4 retention proof, real learning-outcome GA4 events, and later App Check enforcement.
