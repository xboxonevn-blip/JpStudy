...
Current app-coherence audit: `app-coherence-audit-2026-05-17.md`. Phase 0 confirmed 11 shell branches, 68 `GoRoute` declarations, duplicate Home routes (`/`, `/roadmap`, `/today`), dual onboarding gates, split desktop/mobile Home implementations, and a stale content manifest. It corrected one owner-audit assumption: N2/N1 lesson vocab assets do exist and can reach `/lesson/:id` through direct asset fallback, but the content DB lesson query is hardcoded to `minna_*` tags and misses seeded ShinKanzen rows.
