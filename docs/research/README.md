...
Current app-coherence audit: `app-coherence-audit-2026-05-17.md`; Phase 1 content-pipeline follow-up: `app-coherence-phase1-content-pipeline-2026-05-17.md`. Phase 0 confirmed 11 shell branches, 68 `GoRoute` declarations, duplicate Home routes (`/`, `/roadmap`, `/today`), dual onboarding gates, split desktop/mobile Home implementations, and a stale content manifest. Phase 1 made `/lesson/:id` vocab source-aware (`minna_*` for N5/N4, `shinkanzen_*` for N3/N2/N1), regenerated `assets/data/content/index.json` from actual assets, and added guards for manifest drift plus N5-N1 lesson vocab seeding. Remaining content-pipeline caveat: progress terms are still keyed by integer `lessonId`, so level switching can collide across N5/N3/N2/N1 lesson IDs 1-25.
