...
Current D6.Q6.6 status: first min-touch remediation is shipped. `AppTouchTargets.min = 44` now guards compact top-bar notifications, Discover reorder/focus controls, mistake delete buttons, and interactive `StarRating`. Remaining touch work is the lower-risk route-specific sweep for lesson inline zero-constraint icons, library/practice compact action CTAs, handwriting free-practice CTA, and any remaining active `minimumSize: Size(0, 0)` controls.
...
Current D8.Q8.1 blocker: release readiness is gated by hygiene and parity, not build compilation alone. Focused route/layout smoke tests pass after a stale `/exam-center` smoke assertion was updated. Latest secret-backed automated deploy: `316f80c8` deployed through GitHub Actions run `25933463058` on 2026-05-16. Latest live host re-check on 2026-05-16T01:19+07 returned primary `https://jpstudy.web.app/` `200` and legacy `https://jpstudy-v2.web.app/` `404`. Remaining blockers: the first GA4 export sample is too small for outcome claims, Firebase Storage is not provisioned for legacy migration, Sentry lacks a real DSN/first issue, and legal copy still needs human approval.
...
- D6 status update: Q6.6 min-touch policy started with `AppTouchTargets.min = 44` and focused coverage for compact top bar, Discover controls, mistake delete, and interactive stars; remaining work is route-specific compact CTA cleanup.
