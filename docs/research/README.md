...
Current D8.Q8.1 blocker: release readiness is gated by hygiene and parity, not build compilation alone. Focused route/layout smoke tests pass after a stale `/exam-center` smoke assertion was updated. Latest secret-backed automated deploy: `a98fbcec` deployed through GitHub Actions run `25976329271` on 2026-05-17. Latest live host re-check on 2026-05-16T08:05+07 visually verified Kanji radical headers, Han-Viet rules, and Review Forecast labels on `https://jpstudy.web.app/`. Remaining blockers: the first GA4 export sample is too small for outcome claims, Sentry lacks a real DSN/first issue, and legal copy still needs human approval. Firebase Storage is descoped for beta by owner decision on 2026-05-17 because the project stays on Spark and beta backup is local file export/import.
...
- D8 launch-readiness aggregate: `npm run report:launch-readiness -- --json` added on 2026-05-16. Latest Storage-aware run on 2026-05-17T07:13+07 returns `complete=false` without `storage-not-provisioned`; unresolved gates are legal approval, Sentry DSN/first issue, executed deletion proof, GA4 retention proof, BigQuery-exported learning-outcome GA4 rows, and later App Check enforcement. Current launch/storage/deletion/GA4 reports print operator URLs so handoff is direct, but URLs do not close proof gates. Client-side live emission of all three learning events is now proven by Playwright network capture.
