...
- D8 launch-readiness aggregate: `npm run report:launch-readiness -- --json` added on 2026-05-16. Latest Storage-aware run on 2026-05-17T08:27+07 returns `complete=false` without `storage-not-provisioned` or `ga4-learning-events-missing`; unresolved gates are legal approval, Sentry DSN/first issue, executed deletion proof, GA4 retention proof, and later App Check enforcement. Current launch/storage/deletion/GA4 reports print operator URLs so handoff is direct, but URLs do not close proof gates. Client-side live emission and BigQuery export of all three learning events are now proven.
